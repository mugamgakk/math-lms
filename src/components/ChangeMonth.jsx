...
const today = dayjs(new Date()).format("YYYYMM");

function DateNext({onChange, value = new Date(), style}) {

    let dateFormat = dayjs(value).format("YYYY.MM");

    const changeDate = useCallback((num)=>{
        let dayValue = dayjs(value).format("YYYYMM");
        ...
        let {$d : day} = dayjs(value).add(num, "M");
    ...
    return ( 
        <div className='fa' style={style}>
            ...
            <h4 className='next-date'>{dateFormat}</h4>
